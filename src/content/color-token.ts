...
export function createColors(
  ...
  return tokens.map((token) => colorTokenToCSS(token as ColorToken, mappedTokens, tokenGroups)).join('\n');
}

function colorTokenToCSS(token: ColorToken, mappedTokens: Map<string, Token>, tokenGroups: Array<TokenGroup>): string {
...
function tokenVariableName(token: Token, tokenGroups: Array<TokenGroup>): string {
  ...
  return NamingHelper.codeSafeVariableNameForToken(token, StringCase.paramCase, parent, 'color');
}
